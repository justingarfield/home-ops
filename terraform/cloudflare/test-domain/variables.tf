...
variable "onmicrosoft_domain_name" {
  type        = string
  description = "Used in any record that requires your onmicrosoft.com domain name; keeping this value private."
  default     = ""
  validation {
    condition = length(var.onmicrosoft_domain_name) >= 1
    error_message = "onmicrosoft_domain_name must be at-least 1-characters in length (e.g. umbrella-corp)"
  }
}

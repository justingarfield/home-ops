...
resource "hyperv_network_switch" "k8s_staging_workers" {
  name                                    = "K8sStaging-Workers"
  notes                                   = "Kubernetes Staging Environment - Worker Node vSwitch"
  allow_management_os                     = false
  enable_embedded_teaming                 = false
  enable_iov                              = false
  enable_packet_direct                    = false
  minimum_bandwidth_mode                  = "None"
  switch_type                             = "External"
  net_adapter_names                       = ["Broadcom NetXtreme Gigabit Ethernet #3"]
  default_flow_minimum_bandwidth_absolute = 0
  default_flow_minimum_bandwidth_weight   = 0
  default_queue_vmmq_enabled              = false
  default_queue_vmmq_queue_pairs          = 16
...
resource "hyperv_machine_instance" "default" {
  ...
  notes                                   = "Kubernetes Staging Environment - Control Plane 01"
  processor_count                         = 2
  ...
  network_adaptors {
    name                                       = "lan"
    switch_name                                = hyperv_network_switch.k8s_staging_controlplane
    management_os                              = false
    is_legacy                                  = false
